...
func TestBuildTaskContestStandingsZeroPenaltyAndPracticeScores(t *testing.T) {
	...
	b := NewBuilder(nil, log.New(io.Discard, "", 0), 1)
	out := b.buildTaskContestStandings(contest, []domain.Student{{ID: "s1", PublicName: "У"}}, map[string]*accountStatuses{"s1": st}, nil, nil, nil, nil, nil)
	...
	out = b.buildTaskContestStandings(eduContest, []domain.Student{{ID: "s1", PublicName: "У"}}, map[string]*accountStatuses{"s1": st}, nil, nil, nil, nil, nil)
...
func TestBuildTaskContestAcceptedMark(t *testing.T) {
	...
	out = b.buildTaskContestStandings(contest, students, map[string]*accountStatuses{"s1": st2}, nil, nil, nil, nil, nil)
	row = out.Rows[0]
	if row.Accepted == nil || !row.Accepted[0] || row.Accepted[1] {
