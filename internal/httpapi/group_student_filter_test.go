...
func TestTempoLegendMatchesCurrentModel(t *testing.T) {
	...
	for _, want := range []string{
		...
		"сколько делает",             // темп
		"идейность",                  // первая ось сложности задачи
		"реализация",                 // вторая ось
		"берёт ли головой",           // соображает
		"попадает ли с первого раза", // аккуратность
		// Две метрики связаны, и об этом сказано прямо: иначе их читают как
		// независимые оценки и удивляются совпадению.
		"связаны довольно сильно",
	...
	for _, stale := range []string{
		...
		"Чего здесь нет",        // «уровень» теперь измеряется — это «соображает»
		"насколько чисто берёт", // снятая «уверенность»
	} {
