...
```json
{
  ...
  "subcontests": [
    { "title": "Задачи", "tasks": [
      "https://codeforces.com/problemset/problem/4/A",
      "https://informatics.msk.ru/mod/statements/view.php?chapterid=1791"
    ] }
  ]
}
```
...
- **форма «Контесты»** (`/standings/admin/contests`) — конструктор task- и
  provider-контестов (метаданные, `table_name`, окно `start/end`, подконтесты с
  задачами по строкам, `provider_config`);
- **страница группы** (`/standings/admin/group?slug=…`) — удаление участников
  ...
  (ссылки и inline) с флагом `update` и переопределением `table_name` у ссылок,
  кнопка «Добавить ссылку» (выбор из глобального `data/contests.json`) и
  «+ inline-контест» (та же форма контеста, но определение хранится в группе);
