...
- **`"type": "table"`** — берётся значение из сводной вкладки `table_name`
  ...
  - `"max"` — по лучшему участнику группы (топ = 10);
...
- **`"type": "manual"`** — ручные оценки (0–10). Хранятся в
  ...
  Удобнее вводить через админку: у группы кнопка **«Оценки»** открывает сетку
  учеников × ручных столбцов с сохранением. **Важно:** оценки считаются при
  генерации, поэтому изменения (и ручные, и из таблиц) появляются после
  ближайшего `generate`.

Обратная совместимость: нет `grades` / нет `columns` — кнопки и страницы оценок
нет; нет `grades_manual.json` или записи — ручная ячейка пустая; ничего не падает.
...
Окно задаётся **на стороне группы** — в записи `groups/<slug>/contests.json`
(и у ссылки на глобальный контест, и у inline-определения):
...
Так один глобальный контест может идти в разных группах в разное время. Поля
`start_time`/`end_time` в записи-ссылке (как и `table_name`) не превращают её в
inline-определение. Для обратной совместимости окно в самом определении контеста
(глобальном или inline) тоже работает; окно из записи группы приоритетнее.
В админке окно у ссылок правится на странице группы, у inline-контестов — в их
форме; из формы глобальных контестов поля окна убраны (существующие значения при
редактировании не теряются).
...
- **форма «Ученики»** (`/standings/admin/students`) — добавление/редактирование/удаление
  учеников и их аккаунтов (при удалении ученик убирается и из групп);
