...
type GradeColumn struct {
	...
	// IgnoreMissingContests — не учитывать целиком пропущенные контесты (без
	// единой попытки и без баллов) при подсчёте оценки: у каждого ученика
	// знаменатель (нормировка) складывается только из контестов, где он что-то
	// делал. false (по умолчанию/absent) — прежнее поведение: пропущенный
	// контест идёт в оценку как ноль. Только для type=table.
	IgnoreMissingContests bool `json:"ignore_missing_contests,omitempty"`
}
...
type GeneratedTask struct {
	...
	Hidden bool `json:"hidden,omitempty"`
	// Weight — типичное время решения задачи в минутах (медиана активного
...
type StudentCourseStats struct {
	...
	// Mind — «соображает»: шанс взять с первой попытки типичную по ИДЕЙНОСТИ
	// задачу курса, 0..1. Модель Раша по исходу «зачтено с первой посылки»,
	// где трудность задачи — её идейность, а задачи входят с весом тем
	// большим, чем сильнее в них перевешивает идея, а не возня.
	//
	// Здесь раньше была «уверенность», а до неё «сила». Сила вырождалась:
	// почти всё начатое в итоге решают, и 86% учеников получали ровно 100%.
	Mind float64 `json:"mind,omitempty"`
	// Accuracy — «аккуратность»: во сколько раз меньше лишних посылок, чем
	// ожидается на этих задачах. 1,0 — как обычно, 1,5 — в полтора раза
	// чище, 0,7 — в полтора раза грязнее. Считается по задачам с перевесом
	// реализации и с усадкой к 1,0, пока свидетельств мало.
	...
	// Заполняется, только когда глобальная когорта шире состава группы; иначе
	// nil (в профиле тумблер «все группы» показывает те же числа). У самого
	// Global поля Global и Flags не заполняются.
	Global *StudentCourseStats `json:"global,omitempty"`
}
...
// Исход проверки флага преподавателем. По умолчанию (флаг не размечен) эпизоду
// не доверяем: его посылки НЕ учитываются в подсчёте темпа.
const (
	// FlagResolutionLegit — реально сам решил: посылки корректны, эпизод
	// возвращается в подсчёт темпа.
	FlagResolutionLegit = "legit"
	// FlagResolutionTransfer — перенос посылок (например, с другого сайта):
	// посылки корректны, но эпизод остаётся исключённым из темпа.
	FlagResolutionTransfer = "transfer"
	// FlagResolutionViolation — нарушение: эпизод остаётся исключённым из темпа,
	// а отметка остаётся подсвеченной в профиле навсегда.
...
// TaskRating — сложность задачи по двум осям, выставленная по условию (моделью
// или преподавателем) и живущая в data/task_ratings.json.
//
// Оси намеренно разные по смыслу, а не «сложность» и «ещё раз сложность»:
//
//   - ИДЕЙНОСТЬ — надо ли догадаться. Хранится как IdeaRate: доля дошедших до
//     задачи, кто придумает решение САМ.
//   - РЕАЛИЗАЦИЯ — много ли возни, когда идея уже есть. Хранится как
//     ImplAttempts: сколько посылок уйдёт у того, кто УЖЕ придумал.
//
// Обе величины наблюдаемые: их можно сверить с фактом и увидеть, где оценщик
// систематически ошибается. Абстрактную «сложность от 1 до 5» сверить не с чем.
//
// Зачем оценка по условию нужна вообще. По выгрузке курса (378 задач, 130
// учеников, 30 тысяч посылок) эти две оси измеряются из данных ПО-РАЗНОМУ:
//
//   - реализацию данные считают отлично (надёжность 0,92 при делении когорты
//     пополам) — оценка по условию тут слабее (0,62) и нужна только задачам,
//     которые ещё никто не решал;
//   - идейность данные почти не видят (надёжность 0,48 — потолок для любого
//     предсказателя 0,70): бросают задачу не только из-за её трудности, но и
//     потому что ушли из курса. Здесь оценка по условию и есть главный
//     источник, и проверка это подтвердила — смесь с весом ИИ в ~64 ученика
//     бьёт и чистые данные, и чистую оценку.
//
// Отсюда courseIdeaWeight ≫ courseImplWeight в internal/standings.
type TaskRating struct {
	// IdeaRate — ИДЕЙНОСТЬ: какая доля дошедших придумает решение сама, 0..1.
	// Меньше — задача идейнее.
	//
	// Ключ намеренно НЕ solve_rate, хотя раньше поле так и называлось: там
	// лежала доля решивших задачу вообще (на этом курсе 0,85–0,99 с медианой
	// 0,97), и это другая величина. Со старым именем файл прежнего формата
	// загрузился бы молча и был бы прочитан как «почти все придумают сами» —
	// при весе априора в 64 ученика такая ошибка задавила бы данные. Про
	// отказ читать старый формат см. storage.LoadTaskRatings.
	IdeaRate float64 `json:"idea_rate"`
	// ImplAttempts — РЕАЛИЗАЦИЯ: сколько посылок уйдёт у того, кто уже
	// придумал решение, ≥1. Больше — больше технической возни. Раньше поле
	// звалось attempts и считалось по всем, а не только по придумавшим.
	ImplAttempts float64 `json:"impl_attempts"`
...
func (r TaskRating) Valid() bool {
	return r.IdeaRate > 0 && r.IdeaRate < 1 && r.ImplAttempts >= 1
}
...
// Threshold — идейность в шкале модели Раша: при силе ученика θ = 0
// вероятность придумать равна σ(−b), откуда b = −logit(IdeaRate). Это та же
// шкала, в которой считается эмпирическая оценка, поэтому смешивать их можно
// напрямую, без подгонки масштабов.
func (r TaskRating) Threshold() float64 {
	p := math.Max(0.01, math.Min(0.99, r.IdeaRate))
	return -math.Log(p / (1 - p))
}

// Cost — сложность реализации в шкале двусторонней подгонки: логарифм числа
// посылок.
func (r TaskRating) Cost() float64 {
	return math.Log(math.Max(1, r.ImplAttempts))
}

// Опорные точки шкалы 1..10, в которой оси показываются и правятся человеком.
//
// Они заданы ЗНАЧЕНИЯМИ, а не квантилями конкретного курса: иначе один и тот же
// балл означал бы разное в школьном курсе и в олимпиадном, и правка,
// перенесённая между курсами, меняла бы смысл.
//
//	идейность  1 балл — решение придумают сами 97% дошедших
//	идейность 10 баллов — придумает каждый пятый и реже
//	реализация  1 балл — 1,1 посылки у того, кто уже придумал
//	реализация 10 баллов — 2,9 посылки и больше
//
// Диапазон намеренно чуть шире реального разброса: на выгрузке курса (378
// задач) в край упирается 1% задач. Это важно не для красоты — балл, упёршийся
// в край, при сохранении формы записал бы в файл не то, что там лежало.
var (
	ideaScoreLo = -math.Log(0.97 / 0.03) // −logit(0,97)
	ideaScoreHi = -math.Log(0.20 / 0.80) // −logit(0,20)
	implScoreLo = math.Log(1.1)
	implScoreHi = math.Log(2.9)
)

// IdeaScore/ImplScore — оси в баллах 1..10 для показа и правки в админке.
func (r TaskRating) IdeaScore() float64 {
	return scoreFromRange(r.Threshold(), ideaScoreLo, ideaScoreHi)
}

func (r TaskRating) ImplScore() float64 {
	return scoreFromRange(r.Cost(), implScoreLo, implScoreHi)
}

// IdeaFromScore/ImplFromScore — обратный перевод для формы правки.
func IdeaFromScore(score float64) float64 {
	b := valueFromScore(score, ideaScoreLo, ideaScoreHi)
	return 1 / (1 + math.Exp(b))
}

func ImplFromScore(score float64) float64 {
	return math.Exp(valueFromScore(score, implScoreLo, implScoreHi))
}

func scoreFromRange(v, lo, hi float64) float64 {
	if hi <= lo {
		return 1
	}
	s := 1 + 9*(v-lo)/(hi-lo)
...
func valueFromScore(score, lo, hi float64) float64 {
	s := math.Max(1, math.Min(10, score))
	return lo + (s-1)*(hi-lo)/9
}
...
type GeneratedTaskReviewRow struct {
	...
	Solved int `json:"solved"`
	// FactFirstTry — доля взявших задачу С ПЕРВОЙ ПОСЫЛКИ среди пробовавших.
	// Сравнивать идейность с «долей решивших вообще» нельзя: в этом курсе
	// в итоге берут 97% начатого, и такое сравнение всегда показывало бы
