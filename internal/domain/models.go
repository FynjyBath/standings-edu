...
type StudentCourseStats struct {
	...
	// Confidence — шанс, что ученик возьмёт типичную задачу курса с первой
	// попытки (модель Раша по исходу «с первой попытки»), 0..1. Отвечает на
	// «насколько уверенно берёт» — отдельно от «сколько делает».
	//
	// Здесь раньше была «сила» (доля курса, которая по плечу). Она вырождалась:
