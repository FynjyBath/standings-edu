...
- берёт те строки, где число столбцов равно `len(columns)`;
- если таких строк в таблице нет, таблица пропускается.
- каждая подходящая таблица попадает в отдельный `subcontest` внутри одного контеста.
