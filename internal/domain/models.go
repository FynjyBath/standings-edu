...
//
//   - ИДЕЙНОСТЬ — надо ли догадаться. Хранится как SolveRate: доля дошедших до
//     задачи, кто придумает решение САМ.
//   - РЕАЛИЗАЦИЯ — много ли возни, когда идея уже есть. Хранится как Attempts:
//     сколько посылок уйдёт у того, кто УЖЕ придумал.
//
...
type TaskRating struct {
	// SolveRate — ИДЕЙНОСТЬ: какая доля дошедших придумает решение сама, 0..1.
	// Меньше — задача идейнее.
	SolveRate float64 `json:"solve_rate"`
	// Attempts — РЕАЛИЗАЦИЯ: сколько посылок уйдёт у того, кто уже придумал
	// решение, ≥1. Больше — больше технической возни.
...
// вероятность придумать равна σ(−b), откуда b = −logit(SolveRate). Это та же
